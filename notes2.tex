...
\begin{proof}
  ...
  Let $v_1,\ldots,v_{k}$ denote the nodes $R$, ordered so that 
  \[  (v_1,1)\prec (v_2,1)\prec\cdots \prec (v_k,1)\prec (v_k,2)\prec\cdots\prec (v_2,2)\prec (v_1,2) \enspace . \]
  For each $i\in\{1,\ldots,k\}$, let $C_i$ denote the subset of $S'_1$ consisting of children of $v_i$ and, for each $a\in\{1,2\}$, let $(C_i,a)=\{(v,a): v\in C_i\}$.
  
  
  \begin{clm}
    For any $i<j$, any $x\in C_i$, and any $a\in\{1,2\}$, $(x,a)\prec (v_j,1)$ or $(v_j,2)\prec x$.
  \end{clm}
  ...
  For each $i\in\{1,\ldots,k\}$, let $B_i=\{(v,a):a\in\{1,2\}, v\in \{v_i\}\cup C_i\right}$, let $\ell_i=\min_\prec B_i$ and $r_i=\max_\prec B_i$.
  The preceding claim can be strengthened to the following:
  
  \begin{clm}
    For each $1\le i< j\le k$, there is no $z\in B_i$ such that $\ell_j\prec z\prec r_j$.
  \end{clm}
