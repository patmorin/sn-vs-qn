...
\todo{DW: do we actually use the $Q$-edge and $Q$-edge terminology? Are these definitions needed?}
...
& \qquad \cup \{(x,y+1)(x+1,y):x,y\in\{1,\ldots,n-1\}\} \enspace .
\end{align*}
We consider the graph $G:=S_b\CartProd Q_n$. That is, $V(G)=V(S_b)\times V(Q_n)$ where vertices $(v_1,w_1),(v_2,w_2)\in V(G)$ are adjacent whenever $v_1=w_1$ and $v_2w_2\in E(Q_n)$, or $v_1w_1\in E(S_b)$ and $v_2=w_2$.\todo{PM: Suggestion: Replace $Q_n$ with $H$ (for \emph{H}ex). DW: I like this change.}
...
\todo{DW: Why are these $\prec$'s red?}
