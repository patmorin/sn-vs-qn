...
Stacks and queues are fundamental data structures in computer science, but which is more powerful? This question was introduced in 1992 by Heath et al. \cite{HLR92} where they compared the graph parameters \textit{stack-number} and \textit{queue-number} which respectively measure the power of stacks and queues for laying out graphs. 
...
Let $S_b$ be the star graph with root $r$ and $b$ leaves. For $n\in\N$, let $H_n$ be the Hex $n \times n$ grid, defined by $V(H_n):=\{1,\ldots,n\}^2$ and
\begin{align*}
...
\end{align*}
Note that $H_n$ is the dual of the hexagonal grid. 

The graph $G$ in \cref{family} is $S_b \CartProd H_n$ where $b$ and $n$ are chosen to be sufficiently large compared to $s$. 
...
\begin{figure}
	...
	\caption{$S_9 \CartProd Q_4$.}
	\figlabel{graph}
\end{figure}

\subsection*{Subdivisions}

A noteworthy consequence of \Cref{family} is that it resolves a conjecture of \citet{BO99,BO01}. A graph $G'$ is a \textit{subdivision} of a graph $G$ if it can be obtained from $G$ by replacing the edges of $G$ by internally disjoint paths. The vertices of $V(G')\setminus V(G)$ are called \textit{division} vertices. The subdivision of $G$ with exactly $t$ division vertices for each edge is called the \textit{$t$-subdivision} of $G$. \citet{BO99,BO01} conjectured that the stack-number of the 1-subdivision of any graph is not much smaller than that of the original graph. More precisely:

\begin{conj}\label{B_conj}\citep{BO99,BO01}
There exists a function $f$ such that for every graph $G$, if $G'$ is the $1$-subdivision of $G$, then $\sn(G) \leq f(\sn(G’))$. \todo{$\leq 1)$-subdivision?}
\end{conj}
...
\cref{B_conj} is true then stack-number is bounded by queue-number. Since \cref{family} shows that stack-number is not bounded by queue-number, \cref{B_conj} is false. The proof of \citet{DujWoo05} is based on the folllowing key lemma: 
every graph $G$ has a $3$-stack subdivision with $1+2 \ceil{\log_2\qn(G)}$ division vertices per edge. Applying this result to the graph $G=S_b\CartProd H_n$ in \cref{family}, 
the $5$-subdivision of $S_b\CartProd H_n$ has a $3$-stack layout. If \cref{B_conj} was true, then applying it three times would imply that  $S_b\CartProd H_n$ has bounded stack-number, contradicting \cref{family}. 
...
%\todo[inline]{PM: Does anyone know if there is a standard box operator that is typeset like this $S\boxtimes Q$ or $S\boxdot Q$ instead of like this $S\square Q$ or like this $S\Box Q$?  I tried square and Box. DW: I defined \texttt{CartProd} which typesets okay $A \CartProd B$	.}
...
\todo{PM: Suggestion: Replace $Q_n$ with $H_n$ (for \emph{H}ex). DW: I have made this change.}
...
\bigskip We now turn to the proof of our main result, the lower bound on $\sn(S_b\CartProd H_n)$. Consider a hypothetical $s$-stack layout $(\varphi,\prec)$ of $S_b\CartProd H_n$ where $n$ and $b$ are chosen sufficiently large compared to $s$ as detailed below.  We begin with three lemmata that, for sufficiently large $b$, allow us to find a large subgraph $S_d$ of $S_b$ for which the stack layout $(\varphi,\prec)$ of $S_d\CartProd H_n$ is highly structured.
...
For each leaf $v$ in $L$, let $\varphi_v$ be the edge colouring of $H_n$ defined by $\varphi_v(x,y):=\varphi(v,x,y)$. Since $H_n$ has maximum degree $6$, it has less than $3n^2$ edges.  Therefore are fewer than $s^{3n^2}$ edge colourings of $H_n$ using $s$ colours.  Another application of the Pigeonhole Principle proves the following:
...
\begin{lem}\lemlabel{forward_or_backward}
    There exists a sequence $L_3:=u_1,\ldots,u_{b_3}$ with $\{u_1,\ldots,u_{b_3}\}\subseteq L_2$ of length $b_3\ge b_2^{1/2^{n^2-1}}$ such that, for each $p\in V(H_n)$, $(u_1,p)\prec (u_2,p)\prec\cdots\prec (u_{b_3},p)$ or $(u_1,p)\succ (u_2,p)\succ\cdots\succ (u_{b_3},p)$.
\end{lem}
...
\begin{proof}
    ...
    For each $j\in\{i-1,i\}$, let $Q_j:=H_n[\{(v,p_j):v\in A_{i-1}\}]$.\todo{Tiny notation conflict here: $Q_n$, $Q_i$, $Q_{i-1}$. DW: FIxed since $Q_n$ is now $H_n$.}
...
Given the role of cartesian products in our proof, it is natural to ask when is $\sn(G_1\CartProd G_2)$ bounded? Note that $H_n$ is a subgraph of a planar Hamiltonian graphs (namely, $Q_{2n}$), so $\sn(H_n) \leq 2$. So $\sn(G_1\CartProd G_2)$ can be unbounded even when $G_1$ is a star and $\sn(G_2)\leq 2$. 
Since $\sn(G_2)\leq 1$ if and only $G_2$ is outerplanar, the following question naturally arises: Is $\sn(S \CartProd H)$ bounded for every star $S$ and outerplanar graph $H$ with bounded degree? Is $\sn(T \CartProd H)$ bounded for every tree $T$ and outerplanar graph $H$ with bounded degree? The assumption that $H$ has bounded degree is needed since $S_n \CartProd S_n$ contain the 1-subdivision of $K_{n,n}$ ...........

\todo{Does the 1-subdivision of $K_{n,n}$ have unbounded stack-number?}

MENTION RESULTS OF \citet{Pupyrev20} about bipartite graphs.

Our main result implies that $\sn(S\boxtimes P\boxtimes P)$ is unbounded for stars $S$ and paths $P$. 
It is easily seen that $\sn(S\boxtimes P)$ is bounded~\citep{Pupyrev20}. 
The following question naturally arises (independently asked by \citet{Pupyrev20}): 
