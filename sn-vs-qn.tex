...
Another consequence of our result is that it resolves a question of twinwidthIIpaper concerning the novel graph parameter \emph{twin-width} (see twinwidthIpaper for definition and background on twin-width). A graph class $\mathcal{C}$ is called \textit{hereditary} if for every graph $G$ in $\mathcal{C}$, every induced subgraph of $G$ is also contained in $\mathcal{C}$. A hereditary class of graphs $\mathcal{C}$ with bounded twin-width has \textit{bounded sparse twin-width} if there exists an integer $t$ such that no graphs in $\mathcal{C}$ contains $K_{t,t}$ as a subgraph. twinwidthIIpaper proved that graphs with bounded stack-number have bounded sparse twin-width. They conjecture that the inclusion is strict; that is, there exists a hereditary class of graphs with bounded sparse twin-width and unbounded stack-number. We prove that this is so.

We first note that every tree has bounded twin-width (twinwidthIpaper). Let $G$ and $H$ be graphs with bounded twin-width where $H$ also has bounded maximum degree. Then the strong product of $G$ and $H$ also has bounded twin-width (twinwidthIIpaper).\footnote{Define strong product here} In particular, it follows that for all integers $b,n$, the graph $G^{(b,n)}:= S_b \boxtimes P_n \boxtimes P_n$ has bounded twin-width. One can also check that $G^{(b,n)}$ does not contain $K_{10,10}$ as a subgraph. Let $\mathcal{C}$ be the class of graphs that consists of $G^{(b,n)}$ and every induced subgraph of $G^{(b,n)}$ for all integers $b,n$. Then $\mathcal{C}$ is a hereditary class of graphs with bounded sparse twin-width. By \Cref{family}, since $S_b \CartProd H_n \in C$, it follows that $\mathcal{C}$ has unbounded stack-number. So \Cref{family} confirms that bounded sparse twin-width does not coincide with bounded stack-number. It remains open whether bounded sparse twin-width coincides with bounded queue-number.
