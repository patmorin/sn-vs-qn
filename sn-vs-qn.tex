...
\textbf{UPDATE THE FOLLOWING PARAGRAPH WITH PAT'S NOTATION}\todo{PM: We should probably replace $\prec$ and $\succ$ with $<$ and $>$, but I do think the edge colouring $\varphi$ is more convenient for what we're doing.}
...
This demonstrates that stacks are not more powerful than queues for laying out graphs. It remains open whether queues are more powerful than stacks; that is, whether queue-number is bounded by stack-number. Before we specify the graph $G$ in \Cref{family}, we must first introduce graph products.
...
Graph products are a powerful tool in structural graph theory and have been used to solve many open problems concerning queue layouts of graphs \cite{DJMMUW20,DMW}. The graph $G$ in \Cref{family} is obtained as follows (See \figref{graph}): Let $S_b$ denote the star graph with root $r$ and $b$ leaves. For an even positive integer $n$, let $H_n$ be the Hex $n \times n$ grid, defined by $V(H_n):=\{1,\ldots,n\}^2$ and
...
As a consequence of this result, Dujvonic et al. showed that if the Blankeship and Oporowski Conjecture is true, then it follows that stack-number is bounded by queue-number. But as \Cref{family} proves that this is not the case, it follows that \Cref{B_conj} is indeed false. We note that for our graph $G$ in \Cref{family}, since $\qn(G) \leq 4$ as a consequence of \Cref{sub_thm} we have the following corollary.

\begin{cor}
	For the graph $G$ specified in \Cref{family}, there exists a $3$-stack layout of the $5$-subdivision of $G$.
\end{cor}
...
& \qquad \cup \{(x,y+1)(x+1,y):x,y\in\{1,\ldots,n-1\}\} \enspace .
\end{align*}
We consider the graph $G:=S_b\CartProd Q_n$. That is, $V(G)=V(S_b)\times V(Q_n)$ where vertices $(v_1,w_1),(v_2,w_2)\in V(G)$ are adjacent whenever $v_1=w_1$ and $v_2w_2\in E(Q_n)$, or $v_1w_1\in E(S_b)$ and $v_2=w_2$.\todo{PM: Suggestion: Replace $Q_n$ with $H_n$ (for \emph{H}ex). DW: I like this change.}
...
SAY SOMETHING ABOUT THE RESULTS OF \citet{Pupyrev}. I EXPECT WE SOLVE SOME OPEN PROBLEM HERE.\todo{PM:Not really, his open problem is about $H\boxtimes P$ where $H$ has bounded treewidth.}
