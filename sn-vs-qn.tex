...
\end{align*}
We consider the graph $G:=S_b\square Q_n$. See \figref{graph}. That is, $V(G)=V(S_b)\times V(Q_n)$ where vertices $(v_1,w_1),(v_2,w_2)\in V(G)$ are adjacent whenever $v_1=w_1$ and $v_2w_2\in E(Q_n)$, or $v_1w_1\in E(S_b)$ and $v_2=w_2$.\todo{Suggestion: Replace $Q_n$ with $H$ (for \emph{H}ex)}
...
\begin{figure}
	...
	\caption{$S_4\Box Q_3$.\todo[inline]{Add full graph.}}
	\figlabel{graph}
\end{figure}
...
\begin{proof}
	...
	\begin{proof}[Proof of \clmref{interleave}]
		...
		\[
		   ...
		   (v_{j+1}, p_i) \enspace .
	   	\]
		Therefore the edges $(v_j,p_{i-1})(v_j,p_{i})$ and $(v_{j+1},p_{i-1})(v_{j+1},p_i)$ cross.  But this contradicts (P1) and completes the proof of \clmref{interleave}.
	\end{proof}
