...
\todo{Add discussion of result of \citet{BK79}: $\sn(G\CartProd H) \leq \sn(G) + \dsn(H)$ for bipartite $H$. Highlight the key difference between stack and queue layouts is that we need $H$ to be bipartite here.}
...
\todo{There is an annoying notation overlap of $\varphi_v$ and $\varphi_0$. Replace $\varphi_0$ by $\phi$?}

Let $S_{b_2}$ be the subgraph of $S_b$ induced by $L_2\cup\{r\}$. The preceding two lemmata ensure that, for distinct leaves $v$ and $w$ of $S_{b_2}$, the stack layouts of the isomorphic graphs $H^v:=G[\{(v,p):p\in V(H_n)\}]$ and $H^w:=G[\{(w,p):p\in V(H_n)\}]$ are identical. \todo{Do we need $H^v$ and $H^w$?} The next lemma is a statement about the relationships between the stack layouts of $S^p:=G[\{(v,p):v\in V(S_{b_2})\}]$ and $S^q:=G[\{(v,q):v\in V(S_{b_2})\}]$ for  distinct $p,q\in V(H_n)$. 
\todo{To we need $S^p$ and $S^q$?} It does not assert that these two layouts are identical but it does state that they fall into one of two categories.
...
\begin{proof}
    ...
    \todo{This paragraph is notation-heavy. Replace $d_i$ by $|S_i|$?}
\end{proof}

\todo{Replace $b,b_1,b_2,b_3$ by $a,b,c,d$ to avoid double-subscripts?}

Let $d:=b_3$ and let $S_d$ be the the subgraph of $S_b$ induced by $\{r\}\cup\{u_1,\ldots,u_{d}\}$ where $u_1,\ldots,u_d$ is the sequence of leaves defined in \lemref{forward_or_backward}. \todo[inline]{Do we need `` where $u_1,\ldots,u_d$ is the sequence of leaves defined in \lemref{forward_or_backward}''?} Consider the (improper) vertex colouring of $H_n$ obtained by colouring each vertex $p\in V(H_n)$ \emph{red} if $(u_1,p)\prec\cdots\prec (u_d,p)$ and colouring $p$ \emph{blue} if $(u_1,p)\succ\cdots\succ(u_d,p)$. We need the following famous Hex Lemma~\citep{Gale79}.
...
\begin{proof}
	...
	\begin{figure}
		...
		\figlabel{twister}
	\end{figure}
\todo{In figure 1, replace $Z_{n,i}$ by $Z_{n,v_i}$ and replace green dot by $v_1$ in $Z_{n,v_1}$  
}
    ...
    For each $j\in\{i-1,i\}$, let $H^j:=H_n[\{(v,p_j):v\in A_{i-1}\}]$. \todo[inline]{The graph $H^j$ has no edges. All we care about is the vertex set, so let's not bother taking the induced subgraph.}
    ...
    \begin{compactenum}[(P1)]
        ...
        \item By the application of \cref{hex_lemma}, $(v,p_{i-1})\prec (w,p_{i-1})$ if and only if $(v,p_{i})\prec (w,p_{i})$ for each $v,w\in A_{i-1}$.\todo[inline]{Replace ``By the application of \cref{hex_lemma}'' by ``Since $p_{i-1}$ and $p_i$ are red''. In fact, all we need here is that $p_{i-1}$ and $p_i$ are the same colour.}
	...
	\begin{proof}[Proof of \clmref{interleave}]
		By (P3) we may assume, without loss of generality, that $(v,p_{i-1})\prec (v,p_i)$ for each $v\in A_{i-1}$, in which case we are trying to prove the claim for $t=0$.  It is sufficient, therefore to show that $(v_j,p_i)\prec (v_{j+1},p_{i-1})$ for each $j\in\{1,\ldots,r-1\}$.  For the sake of contradiction, suppose $(v_j,p_{i})\succ (v_{j+1},p_{i-1})$ for some $j\in\{1,\ldots,r-1\}$. By definition \todo{Replace ``By definition'' by ``By the labelling of $A_{i-1}$''} $(v_j,p_{i-1})\prec (v_{j+1},p_{i-1})$ so, by (P2),  $(v_{j},p_i) \prec (v_{j+1},p_i)$.  Therefore
		\[
