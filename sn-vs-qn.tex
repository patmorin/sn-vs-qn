...
\begin{lem}\lemlabel{ramsey-tree}
...
% Although $H$ is not a tree we will still use tree terminology (leaves, internal nodes, parents, chilren, etc) for its nodes.  When we do this we are referring to the roles these nodes play in the tree $T$.
...
\begin{lem}\lemlabel{forward-or-backward}
...
Starting at the root $v$ of $T$ apply \lemref{forward-or-backward} to obtain the sequence of $d$ children $v_1,\ldots,v_d$.  Call these children \emph{keepers} and recursively apply this process to each non-leaf keeper.  At the end of this process the set of keepers induces a complete $d$-ary tree of height $h$.
...
By applying \lemref{ramsey-tree} with $c:=
...
\begin{lem}\lemlabel{two-baby-piles}
...
\begin{proof}[Proof Sketch]
    ...
    What remains is to prove that the sets $X$ and $Y$ are separated with respect to $\prec$.  For horizontal edges, this is already taken care of by \lemref{two-baby-piles}.  For vertical edges, there are two cases to consider:
    \begin{enumerate}
...
To summarize, \lemref{two-piles} shows that we can find a stack layout of a subdivision of $T$ that is spread across two piles $X$ and $Y$ that are separated with respect to $\prec$.  In fact, in each pile, we can find at least one copy of each node of $T$.
...
\begin{proof}
    For each $i\in\{1,\ldots,h\}$, the path $R$ contains a at least one horizontal edge $(i,a)(i,a')$.  The proof of \lemref{two-piles} places one endpoint of this edge in $A$ and the other endpoint in $B$.  The endpoint in $A$ produces a copy of all depth-$i$ nodes in $X$ and the copy in $B$ produces a copy of all depth-$i$ nodes in $Y$.  Each of these includes a copy of $v$.
\end{proof}
