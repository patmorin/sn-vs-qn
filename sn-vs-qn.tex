...
\begin{proof}
	Extend the total order $\prec$ onto a partial order over subsets of $V(G)$ so that, for $V,W\subseteq V(G)$,  $V\prec W$ if and only if $v\prec w$ for each $v\in V$, and $w\in W$.  We abuse notation slightly by using $\prec$ to compare elements of $V(G)$ and subsets of $V(G)$ so that, for $v\in V(G)$ and $V\subseteq V(G)$, $v\prec V$ denotes $\{v\}\prec V$.
