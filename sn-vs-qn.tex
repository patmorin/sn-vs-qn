...
\todo{Do we want a figure showing an example of a stack layout and a queue layout here? $H_4$ would be a good example. }
...
\begin{figure}[H]
	...
	\caption{A $2$-queue layout and a $2$-stack layout of $H_4$. Edges drawn above vertices are assigned to the first queue/stack and edges drawn below vertices are assigned to the second queue/stack.}
	\label{layouts}
\end{figure}
...
Recall that every 1-queue graph has a 2-stack layout \citep{HLR92} and we proved that there are 4-queue graphs with unbounded stack-number. The following questions remain open: Do 2-queue graphs have bounded stack-number? Do 3-queue graphs have bounded stack-number? \todo[inline]{Do we want to make a conjecture here? I would say ``no'' for both questions.}
%DW: In fact, here is a possible graph. Let $G:= S_b\CartProd H_n$ and let $G'$ be the 5-subdivision of $G$. Since $\qn(G)\leq 4$ by a result of \citet{DujWoo05}, we have $\qn(G')\leq 2$. I suspect that $\sn(G')$ is still unbounded. A closer look at the proof of \citet{DujWoo05} might mean we don't have to subdivide so much???
