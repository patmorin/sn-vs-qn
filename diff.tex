...
Note that stack layouts are equivalent to book embeddings (first defined by \citet{Ollmann73} in 1973), and stack-number is also known as \emph{page-number}, \emph{book-thickness} or \emph{fixed outer-thickness}. Stack and queue layouts have other applications including computational complexity~\citep{GKS89,DSW16,Bourgain09,BY13},  RNA folding~\citep{HS99}, graph drawing in two~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{BB04,ADFPR12,SSSV-WG94} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{BB04,ADFPR12,SSSV-JGT96} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend and three dimensions~\citep{DMW05,Wood-GD01,DMW17,DPW04},
...
This paper considers the first of the \DIFdelbegin \DIFdel{above questions }\DIFdelend \DIFaddbegin \DIFadd{questions from the start of the paper}\DIFaddend . In a positive direction, \citet{HLR92}  showed that every 1-queue graph has a $2$-stack layout. On the other hand, they described graphs that need exponentially more stacks than queues. In particular, $n$-vertex ternary hypercubes have queue-number $O(\log n)$ and stack-number $\Omega(n^{1/9-\epsilon})$ for any $\epsilon>0$.
...
We now show that $\qn(S_b\CartProd H_n)\leq 4$, which with \cref{Main} implies \cref{family}. We need the following definition due to \citet{Wood-Queue-DMTCS05}. A queue layout $(\varphi,\prec)$ is \emph{strict} if for every vertex $u\in V(G)$ and for all neighbours $v,w\in N_G(u)$, if \DIFdelbegin \DIFdel{$u\prec v,w$ or $v,w \prec u$}\DIFdelend \DIFaddbegin \DIFadd{$u\prec v\prec w$ or $v\prec w \prec u$}\DIFaddend , then $\varphi(uv)\neq \varphi(uw)$. Let $\sqn(G)$ be the minimum integer $k$ such that $G$ has a strict $k$-queue layout. To see that $\sqn(H_n) \leq 3$, order the vertices row-by-row and then left-to-right within a row, with vertical edges in one queue, horizontal edges in one queue, and diagonal edges in another queue\DIFdelbegin \DIFdel{(this construction puts the edges below the vertices in \mbox{%DIFAUXCMD
\cref{layouts} }\hspace{0pt}%DIFAUXCMD
into two queues)}\DIFdelend . \citet{Wood-Queue-DMTCS05} proved that for all graphs $G_1$ and $G_2$,
\begin{equation}
\label{QueueProduct}
...
\end{equation}
The key difference between \cref{QueueProduct} and \cref{StackProduct} is that $G_2$ is assumed to be bipartite in \cref{StackProduct}. \cref{Main} says that this assumption is essential, since it is easily seen that $(\dsn(H_n))_{n\in\N}$ is bounded, but the stack number of $(S_b \CartProd H_n)_{b,n\in\N}$ is unbounded by \cref{Main}. \DIFdelbegin \DIFdel{The key property of }\DIFdelend \DIFaddbegin \DIFadd{We choose }\DIFaddend $H_n$ \DIFdelbegin \DIFdel{is that }\DIFdelend \DIFaddbegin \DIFadd{in \mbox{%DIFAUXCMD
\cref{Main} }\hspace{0pt}%DIFAUXCMD
since }\DIFaddend it satisfies the Hex Lemma (\cref{hex_lemma} below), which quantifies the intuition that $H_n$ is far from being bipartite \DIFaddbegin \DIFadd{(while still having bounded queue-number and bounded maximum degree so that \mbox{%DIFAUXCMD
\cref{QueueProduct} }\hspace{0pt}%DIFAUXCMD
is applicable)}\DIFaddend . 
...
\begin{proof}
    ...
    Begin with the sequence \DIFdelbegin \DIFdel{$S_1:=v_{1,1},\ldots,v_{1,c}$ }\DIFdelend \DIFaddbegin \DIFadd{$V_1:=v_{1,1},\ldots,v_{1,c}$ }\DIFaddend that contains all $c$ elements of $L_2$ ordered so that $(v_{1,1},p_1)\prec\cdots\prec(v_{1,c},p_1)$.  For each $i\in\{2,\ldots,n^2\}$, the Erd\H{o}s-Szekeres Theorem~\citep{ES35} implies that \DIFdelbegin \DIFdel{$S_{i-1}$ }\DIFdelend \DIFaddbegin \DIFadd{$V_{i-1}$ }\DIFaddend contains a subsequence \DIFdelbegin \DIFdel{$S_i:=v_{i,1},\ldots,v_{i,|S_i|}$ of length $|S_i|\ge \sqrt{|S_{i-1}|}$ such that $(v_{i,1},p_i)\prec\cdots\prec(v_{i,|S_i|},p_i)$ or $(v_{i,1},p_i)\succ\cdots\succ(v_{i,|S_i|},p_i)$}\DIFdelend \DIFaddbegin \DIFadd{$V_i:=v_{i,1},\ldots,v_{i,|V_i|}$ of length $|V_i|\ge \sqrt{|V_{i-1}|}$ such that $(v_{i,1},p_i)\prec\cdots\prec(v_{i,|V_i|},p_i)$ or $(v_{i,1},p_i)\succ\cdots\succ(v_{i,|V_i|},p_i)$}\DIFaddend .  It is straightforward to verify by induction on $i$ that \DIFdelbegin \DIFdel{$|S_i| \ge c^{1/2^{i-1}}$ }\DIFdelend \DIFaddbegin \DIFadd{$|V_i| \ge c^{1/2^{i-1}}$ }\DIFaddend resulting in a final sequence \DIFdelbegin \DIFdel{$S_{n^2}=:L_3$ }\DIFdelend \DIFaddbegin \DIFadd{$V_{n^2}$ }\DIFaddend of length at least $c^{1/2^{n^2-1}}$.    
\end{proof}
...
%DIF < Every red--blue vertex colouring of the graph $H_n$	contains an $n$-vertex path $R$ consisting entirely of red vertices or entirely of blue vertices.
...
\begin{proof}
    ...
    Before defining $A_1,\ldots,A_n$ we first show how the existence of the set $A_n$ implies the lemma.  To avoid triple-subscripts, let $d':=d_n\ge d/2^{n-1}$. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{By (C2), the }\DIFaddend set $A_n$ defines vertex sets $Z_{n,v_1}\prec\cdots\prec Z_{n,v_{d'}}$ (see \cref{fig_twister}). \DIFdelbegin \DIFdel{Recall that }\DIFdelend \DIFaddbegin \DIFadd{The root }\DIFaddend $r$ \DIFdelbegin \DIFdel{is the root }\DIFdelend of $S_b$ \DIFdelbegin \DIFdel{so it }\DIFdelend is adjacent to each of $v_{1},\ldots,v_{d'}$ in $S_d$. \DIFdelbegin \DIFdel{Therefore}\DIFdelend \DIFaddbegin \DIFadd{Thus}\DIFaddend , for each $j\in\{1,\ldots,n\}$ and each $i\in\{1,\ldots,d'\}$, the edge $(r,p_j)(v_i,p_j)$ is in $X$. \DIFdelbegin \DIFdel{Therefore}\DIFdelend \DIFaddbegin \DIFadd{Hence}\DIFaddend , $(r,p_j)$ is adjacent to an element of each of $Z_{n,v_1},\ldots,Z_{n,v_{d'}}$.
	\begin{figure}[!h]
   ...
   \DIFdelbegin \DIFdel{when viewed from afar}\DIFdelend \DIFaddbegin \DIFadd{if we imagine identifying the vertices in each set $Z_{n,v_i}$}\DIFaddend , this situation looks like a complete bipartite graph $K_{n,d'}$ with the root vertices $L:=\{(r,p_j):j\in\{1,\ldots,n\}\}$ in one part and the groups $R:=Z_{n,v_1}\cup\cdots\cup Z_{n,v_{d'}}$ in the other part.  Any linear ordering of $K_{n,d'}$ has a large set of pairwise crossing edges so, intuitively, the induced subgraph $X[L\cup R]$ should also have a large set of pairwise crossing edges. We can formalize this as follows: Label the vertices in $L$ as $r_1,\ldots,r_n$ so that $r_1\prec \cdots\prec r_{n}$.  Then at least one of the following two cases applies (see \figref{median}):
    \begin{enumerate}
    ...
    \DIFdelbegin \DIFdel{All that remains is }\DIFdelend \DIFaddbegin \DIFadd{It remains }\DIFaddend to define the sets $A_1\supseteq\cdots\supseteq A_n$ that satisfy (C1) and (C2).  Let $A_1$ be the set of all the leaves of $S_d$.  For each $i\in\{2,\ldots,n\}$, \DIFaddbegin \DIFadd{assuming that $A_{i-1}$ is already defined, }\DIFaddend the set $A_i$ is defined as follows: \DIFdelbegin \DIFdel{Let $Z_1,\ldots,Z_{|A_{i-1}|}$ }\DIFdelend \DIFaddbegin \DIFadd{For brevity, let $m:= |A_{i-1}|$. Let $Z_1,\ldots,Z_m$ }\DIFaddend denote the sets $Z_{i-1, v}$ for each $v\in A_{i-1}$ ordered so that \DIFdelbegin \DIFdel{$Z_1\prec\cdots\prec Z_{|A_{i-1}|}$}\DIFdelend \DIFaddbegin \DIFadd{$Z_1\prec\cdots\prec Z_m$}\DIFaddend . By Property (C2), this is always possible. Label the vertices of $A_{i-1}$ as \DIFdelbegin \DIFdel{$v_1,\ldots,v_{|A_{i-1}|}$ so that $(v_1,p_{i-1})\prec\cdots\prec (v_r,p_{i-1})$}\DIFdelend \DIFaddbegin \DIFadd{$v_1,\ldots,v_m$ so that $(v_1,p_{i-1})\prec\cdots\prec (v_m  ,p_{i-1})$}\DIFaddend .   (This is equivalent to naming them so that $(v_j,p_{i-1})\in Z_j$ for each \DIFdelbegin \DIFdel{$j\in\{1,\ldots,|A_{i-1}|\}$}\DIFdelend \DIFaddbegin \DIFadd{$j\in\{1,\ldots,m\}$}\DIFaddend .)  Define the set \DIFdelbegin \DIFdel{$A_i:=\{v_{2k+1}:k\in\{0,\ldots,\lfloor(|A_{i-1}|-1)/2\rfloor\}\}=\{v_{j}\in A_{i-1}$}\DIFdelend \DIFaddbegin \DIFadd{$A_i:=\{v_{2k+1}:k\in\{0,\ldots,\lfloor(m-1)/2\rfloor\}\}=\{v_{j}\in A_{i-1}:\text{$}\DIFaddend $j is odd}\}$.  This completes the definition of $A_1,\ldots,A_n$.

	\DIFdelbegin \DIFdel{All that remains is to }\DIFdelend \DIFaddbegin \DIFadd{We now }\DIFaddend verify that $A_i$ satisfies (C1) and (C2) for each $i\in\{1,\ldots,n\}$.  We do this by induction on $i$. The base case $i=1$ is trivial, so now assume that $i\in\{2,\ldots,n\}$.   To see that $A_i$ satisfies (C1) observe that $|A_i|=\lceil |A_{i-1}|/2\rceil \ge |A_{i-1}|/2\ge d/2^{i-1}$, where the final inequality follows by applying the inductive hypothesis $|A_{i-1}|\ge d/2^{i-2}$.  Now \DIFdelbegin \DIFdel{all that remains is }\DIFdelend \DIFaddbegin \DIFadd{it remains }\DIFaddend to show that $A_i$ satisfies (C2). \DIFaddbegin \DIFadd{Again, let $m:= |A_{i-1}|$. 
}\DIFaddend 

    Recall that, for each $v\in A_{i-1}$, the edge $e_v:=(v,p_{i-1})(v,p_i)$ is in $X$.  We have the following properties:
    \begin{compactenum}[(P1)]
        \item By \cref{uniform_colour}, $\varphi(e_v)=\phi(p_{i-1}p_i)$ for each $v\in A_{i-1}$\DIFdelbegin \DIFdel{.
        }\DIFdelend \DIFaddbegin \DIFadd{, 
        }\DIFaddend \item Since $p_{i-1}$ and $p_i$ are both red,  \DIFaddbegin \DIFadd{for each $v,w\in A_{i-1}$, we have }\DIFaddend $(v,p_{i-1})\prec (w,p_{i-1})$ if and only if $(v,p_{i})\prec (w,p_{i})$\DIFdelbegin \DIFdel{for each $v,w\in A_{i-1}$}\DIFdelend .
...
We claim that these three conditions imply that the vertex sets $\{(v,p_{i-1}):v\in A_{i-1}\}$ and $\{(v,p_i):v\in A_{i-1}\}$ interleave perfectly with respect to $\prec$.     More precisely:
	...
	\DIFaddend \begin{clm}\clmlabel{interleave} 
		\DIFdelbegin \DIFdel{$(v_1,p_{i-1+t})\prec (v_1,p_{i-t}) \prec (v_2,p_{i-1+t}) \prec (v_2,p_{i-t}) \cdots \prec (v_r,p_{i-1+t}) \prec (v_r,p_{i-t})$ }\DIFdelend \DIFaddbegin \DIFadd{$(v_1,p_{i-1+t})\prec (v_1,p_{i-t}) \prec (v_2,p_{i-1+t}) \prec (v_2,p_{i-t}) \cdots \prec (v_m,p_{i-1+t}) \prec (v_m,p_{i-t})$ 
		}\DIFaddend for some $t\in\{0,1\}$.
	\end{clm}
	\DIFaddbegin 

	\DIFaddend \begin{proof}[Proof of \clmref{interleave}]
		By (P3) we may assume, without loss of generality, that $(v,p_{i-1})\prec (v,p_i)$ for each $v\in A_{i-1}$, in which case we are trying to prove the claim for $t=0$.  Therefore, it is sufficient to show that $(v_j,p_i)\prec (v_{j+1},p_{i-1})$ for each \DIFdelbegin \DIFdel{$j\in\{1,\ldots,r-1\}$}\DIFdelend \DIFaddbegin \DIFadd{$j\in\{1,\ldots,m-1\}$}\DIFaddend .  For the sake of contradiction, suppose $(v_j,p_{i})\succ (v_{j+1},p_{i-1})$ for some \DIFdelbegin \DIFdel{$j\in\{1,\ldots,r-1\}$}\DIFdelend \DIFaddbegin \DIFadd{$j\in\{1,\ldots,m-1\}$}\DIFaddend . By the labelling of $A_{i-1}$,  $(v_j,p_{i-1})\prec (v_{j+1},p_{i-1})$ so, by (P2),  $(v_{j},p_i) \prec (v_{j+1},p_i)$.  Therefore
		\[
...
%DIF <  \todo{DW: Why are these $\prec$'s red?  PM: Just me keeping track of which one was the assumption, they don't need to be red.}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend We now complete the proof that $A_i$ satisfies (C2). Apply \clmref{interleave} and assume without loss of generality that $t=0$, so that
	\[
		(v_1,p_{i-1})\prec (v_1,p_{i}) \prec (v_2,p_{i-1}) \prec (v_2,p_{i}) \cdots \prec (v\DIFdelbegin \DIFdel{_r}\DIFdelend \DIFaddbegin \DIFadd{_m}\DIFaddend ,p_{i-1}) \prec (v\DIFdelbegin \DIFdel{_r}\DIFdelend \DIFaddbegin \DIFadd{_m}\DIFaddend ,p_{i}) \enspace .
	\]

    For each \DIFdelbegin \DIFdel{$j\in\{1,\ldots,r-2\}$}\DIFdelend \DIFaddbegin \DIFadd{$j\in\{1,\ldots,m-2\}$}\DIFaddend , we have $(v_{j+1},p_{i-1})\in Z_{j+1}\prec Z_{j+2}$, so  $(v_j,p_i)\prec (v_{j+1},p_{i-1}) \prec Z_{j+2}$.  Therefore $Z_j\cup\{(v_j,p_i)\} \prec Z_{j+2}$.  By a symmetric argument, $Z_j\cup\{(v_j,p_i)\} \succ Z_{j-2}$ for each  \DIFdelbegin \DIFdel{$j\in\{3,\ldots,r\}$}\DIFdelend \DIFaddbegin \DIFadd{$j\in\{3,\ldots,m\}$}\DIFaddend .  Finally, since $(v_{j},p_i)\prec (v_{j+2},p_i)$ for each odd \DIFdelbegin \DIFdel{$i\in\{1,\ldots,r\}$}\DIFdelend \DIFaddbegin \DIFadd{$i\in\{1,\ldots,m\}$}\DIFaddend , we have $Z_{j}\cup\{(v_j,p_i)\} \prec Z_{j+2}\cup\{(v_{j+2},p_i)\}$ for each odd \DIFdelbegin \DIFdel{$j\in\{1,\ldots,r-2\}$}\DIFdelend \DIFaddbegin \DIFadd{$j\in\{1,\ldots,m-2\}$}\DIFaddend .  Thus $A_i$ satisfies (C2) since the sets \DIFdelbegin \DIFdel{$Z_1\cup\{(v_1,p_i)\},Z_3\cup\{(v_3,p_i)\},\ldots,Z_{2\lfloor (r-1)/2\rfloor+1} \cup (v_{2\lfloor (r-1)/2\rfloor+1},p_i)$ }\DIFdelend \DIFaddbegin \DIFadd{$Z_1\cup\{(v_1,p_i)\},Z_3\cup\{(v_3,p_i)\},\ldots,Z_{2\lfloor (m-1)/2\rfloor+1} \cup (v_{2\lfloor (m-1)/2\rfloor+1},p_i)$ }\DIFaddend are precisely the sets $Z_{i,1},\ldots,Z_{i,d_i}$ determined by our choice of $A_i$.
\end{proof}
...
Our main result also resolves a question of \citet{BGKTWb} concerning \emph{sparse twin-width}; see \citep{BKTW,BGKTWa,BGKTWb} for the definition and background on (sparse) twin-width. \citet{BGKTWb} proved that graphs with bounded stack-number have bounded sparse twin-width, and they write that they ``believe that the inclusion is strict''; that is, there exists a class of graphs with bounded sparse twin-width and unbounded stack-number. \Cref{Main} confirms this intuition, since \DIFdelbegin \DIFdel{it follows from \mbox{%DIFAUXCMD
\citep[Theorem~10]{BGKTWb} }\hspace{0pt}%DIFAUXCMD
that }\DIFdelend the class of all subgraphs of $(S_b \CartProd H_n)_{b,n\in\N}$ has bounded sparse twin-width \DIFdelbegin \footnote{\DIFdel{For graph classes $\mathcal{G}$ and $\mathcal{H}$, let $\mathcal{G}\boxtimes \mathcal{H} := \{ G\boxtimes H: G \in \mathcal{G}, H\in \mathcal{H}\}$. If $\mathcal{G}$ and $\mathcal{H}$ both have bounded twin-width, $\mathcal{H}$ has bounded maximum degree, and no graph in $\mathcal{G}\boxtimes \mathcal{H}$ contains a $K_{t,t}$ subgraph for some constant $t$, then \mbox{%DIFAUXCMD
\citet[Theorem~10]{BGKTWb} }\hspace{0pt}%DIFAUXCMD
proved that the class of all subgraphs of graphs in $\mathcal{G}\boxtimes \mathcal{H}$ has bounded sparse twin-width. Apply this result with $\mathcal{G}:= (S_b)_{b\in \N}$ and $\mathcal{H}:= (H_n)_{n\in\N}$. Every star graph has twin-width $0$ and planar graphs have bounded twin-width~\mbox{%DIFAUXCMD
\citep{BKTW}}\hspace{0pt}%DIFAUXCMD
. Since $S_b \boxtimes H_n$ is 7-degenerate, it contains no $K_{8,8}$ subgraph. Thus the class of all subgraphs of graphs in $\mathcal{G} \boxtimes \mathcal{H}$ has bounded sparse twin-width. In particular, the class of all subgraphs of $(S_b \CartProd H_n)_{b,n\in\N}$ has bounded sparse twin-width.}}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{. 
}\DIFdelend \DIFaddbegin \DIFadd{(since \mbox{%DIFAUXCMD
\citet{BGKTWb} }\hspace{0pt}%DIFAUXCMD
showed that any hereditary class of graphs with bounded queue-number has bounded sparse twin-width). 
%DIF > it follows from \citep[Theorem~10]{BGKTWb} that the class of all subgraphs of $(S_b \CartProd H_n)_{b,n\in\N}$ has bounded sparse twin-width\footnote{For graph classes $\mathcal{G}$ and $\mathcal{H}$, let $\mathcal{G}\boxtimes \mathcal{H} := \{ G\boxtimes H: G \in \mathcal{G}, H\in \mathcal{H}\}$. If $\mathcal{G}$ and $\mathcal{H}$ both have bounded twin-width, $\mathcal{H}$ has bounded maximum degree, and no graph in $\mathcal{G}\boxtimes \mathcal{H}$ contains a $K_{t,t}$ subgraph for some constant $t$, then \citet[Theorem~10]{BGKTWb} proved that the class of all subgraphs of graphs in $\mathcal{G}\boxtimes \mathcal{H}$ has bounded sparse twin-width. Apply this result with $\mathcal{G}:= (S_b)_{b\in \N}$ and $\mathcal{H}:= (H_n)_{n\in\N}$. Every star graph has twin-width $0$ and planar graphs have bounded twin-width~\citep{BKTW}. Since $S_b \boxtimes H_n$ is 7-degenerate, it contains no $K_{8,8}$ subgraph. Thus the class of all subgraphs of graphs in $\mathcal{G} \boxtimes \mathcal{H}$ has bounded sparse twin-width. In particular, the class of all subgraphs of $(S_b \CartProd H_n)_{b,n\in\N}$ has bounded sparse twin-width.}. 
}\DIFaddend It remains open whether bounded sparse twin-width coincides with bounded queue-number.
