...
\begin{lem}\lemlabel{star-ii}
  For every $s,t,r\in\N$, $r>s$, there exists $d\in\N$ such that, every $s$-stack layout $(\prec,\varphi)$ of $S_d\boxtimes P_r$, there exists a $t$-subset $D\subset[d]$ of the leaves of $S_d$ such that:
  \begin{compactenum}
    \item For every $y\in\{1,\ldots,r-1\}$, $\prec$ is a x-major ordering or a y-major ordering of $D\times\{y,y+1\}$.
    \item For every $y\in\{1,\ldots,r-s-1\}$ there exists at least one $j\in\{0,\ldots,s\}$ such that $\prec$ is a y-major ordering of $D\times\{y+j,y+j+1\}$.
  \end{compactenum}
\end{lem}
...
\begin{lem}\lemlabel{dichotomy}
  ...
  \begin{compactenum} 
    \item There exists a $\ceil{k/2}$-element subset $D\subseteq X$ such that $\prec$ is a y-major ordering of $D\times\{0,1\}$.
    \item There exists an $\floor{m/k}$-element subset $D\subseteq X$ such that $\prec$ is a x-major ordering of $D\times\{0,1\}$.
  \end{compactenum}
\end{lem}
...
\begin{proof}[Proof of \lemref{star-ii}]
  Starting with $D_0=[d]$, iteratively apply \lemref{dichotomy} to $\prec$ and the set $S=D_{i-1}\times\{i,i+1\}$ to obtain a set $D_i$ for $i=1,\ldots,r-1$. The final set $D_{r-1}$ has size at least $d^{1/2^{r-1}}$ and satisfies Property~1 of the lemma.  To see that it satisfied Property~2 argue that, otherwise, we will find a twist of size $t$.
\end{proof}
