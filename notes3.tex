...
\begin{proof}
  Note: This is just proof sketch that was written as the ideas were developing.  The actual proof should be by lexicographic induction on the pair $(h,k)$, $h\in\N$, $k\in\{1,\ldots,h\}$, to show that, we can always find a tree that for which $\prec$ is $i$-LCA-determined for each $i\in\{h-k,h-k+1,\ldots,h-1\}$.
  
  The proof is by induction on $h$.  The base case, $h=1$ (where $T$ is a star with $b$ leaves), is trivial.  Indeed, we need only show that $\prec$ is $0$-LCA-determined.  For any two distinct leaves $x$ and $y$, the depth-$1$ ancestors of $x'$ and $y'$ of $x$ and $y$ are $x'=x$ and $y'=y$, respectively.  Therefore $x\prec y$ if and only if $x'\prec y'$ so $\prec$ is $0$-LCA consistent, as required.
